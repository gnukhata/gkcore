    ...
    def gkUpgrade(self):
        ...
        try:
            ...
            if not tableExists("invoicebin"):
                self.con.execute("create table invoicebin(invid serial, invoiceno text NOT NULL, invoicedate  timestamp NOT NULL, taxflag integer default 22, contents jsonb, issuername text, designation text, tax jsonb, cess jsonb, amountpaid numeric(13,2) default 0.00, invoicetotal numeric(13,2) NOT NULL, icflag integer default 9, taxstate text, sourcestate text, orgstategstin text, attachment json, attachmentcount integer default 0, orderid integer,goid integer,orgcode integer NOT NULL, custid intger, consignee jsonb, freeqty jsonb, reversecharge text, bankdetails jsonb, transportationmode text,vehicleno text, dateofsupply timestamp, discount jsonb, paymentmode integer default 2,address text, inoutflag integer,invoicetotalword text, primary key(invid),foreign key(orderid) references purchaseorder(orderid),foreign key(goid) references godown(goid), foreign key(orgcode) references organisation(orgcode) ON DELETE CASCADE, foreign key(orgcode) references customerandsupplier(orgcode), unique(invoiceno,custid,icflag))")
                self.con.execute("create index invoice_orgcodeindex on invoicebin using btree(orgcode)")
                self.con.execute("create index invoice_invoicenoindex on invoicebin using btree(invoiceno)")

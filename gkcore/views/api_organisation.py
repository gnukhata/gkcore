		...
		try:
			...
			if result.rowcount==1:
				...
				try:
					...
					userdata["userrole"] = -1
					result = self.con.execute(gkdb.users.insert().values(username=userdata["username"], userrole=userdata["userrole"], userquestion=userdata["userquestion"], useranswer=userdata["useranswer"], userpassword=userdata["userpassword"]))
					if result.rowcount==1:

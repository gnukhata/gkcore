    ...
    def gkUpgrade(self):
        ...
        try:
            ...
            if not tableExists("budget"):
                ...
                #In Below queries we are creating new table delchalbin which is act as bin for cancelled deliverynotes. 
            if not tableExists("delchalbin"):
                self.con.execute("create table delchalbin(dcid serial, dcno text NOT NULL, dcdate timestamp NOT NULL, dcflag integer NOT NULL, taxflag integer default 22, contents jsonb, tax jsonb, cess jsonb, issuername text, designation text, noofpackages integer NOT NULL, modeoftransport text, attachment json, consignee jsonb, taxstate text,sourcestate text, orgstategstin text, freeqty jsonb, discount jsonb, vehicleno text, dateofsupply timestamp, delchaltotal numeric(13,2) NOT NULL, attachmentcount integer default 0, orgcode integer NOT NULL, custid integer, orderid integer, inoutflag integer NOT NULL, roundoffflag integer default 0, primary key(dcid), foreign key(orderid) references purchaseorder(orderid), foreign key(custid) references customerandsupplier(custid), foreign key(orgcode) references organisation(orgcode) ON DELETE CASCADE)")

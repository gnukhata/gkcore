...
def billwiseEntryLedger(con, orgcode, vouchercode, invid, drcrid):
    try:
        ...
        dcinfo = ""
        if invid == None:
            billdetl = con.execute("select invid, adjamount from billwise where vouchercode = %d and orgcode =%d"%(vouchercode, orgcode)) 
            billDetails=billdetl.fetchall()
            if len(billDetails) != 0:
                inno = "Invoice Nos.:"
                cmno = "Cash Memo Nos.:"
                for inv in billDetails:
                    invno = con.execute("select invoiceno, icflag from invoice where invid = %d and orgcode = %d"%(inv["invid"],orgcode))
                    invinfo= invno.fetchone()
                    if invinfo["icflag"] == 9:
                        inno += str(invinfo["invoiceno"]) +":" +str(inv["adjamount"])+ ","
                        dcinfo = inno
                    else:
                        cmno += str(invinfo["invoiceno"]) +":" +str(inv["adjamount"])+ ","
                        dcinfo = cmno
        else: 
            invno = con.execute("select  invoiceno, icflag from invoice where invid = %d and orgcode = %d"%(invid,orgcode))
            invinfo = invno.fetchone()
            if invinfo["icflag"] == 9:
        ...
        if drcrid != None:
            drcrno = con.execute("select drcrno, dctypeflag from drcr where drcrid = %d and orgcode = %d"%(drcrid,orgcode))
...
def stockonhandfun(orgcode, productCode,endDate):
    try:
        ...
        totaloutward = 0.00
        if productCode != "all":
            openingStockResult = con.execute(select([product.c.openingstock,product.c.productdesc]).where(and_(product.c.productcode == productCode, product.c.gsflag == 7 ,product.c.orgcode == orgcode)))
            osRow =openingStockResult.fetchone()
            openingStock = osRow["openingstock"]
            prodName = osRow["productdesc"]
            stockRecords = con.execute(select([stock]).where(and_(stock.c.productcode == productCode,stock.c.orgcode == orgcode, or_(stock.c.dcinvtnflag != 20,stock.c.dcinvtnflag != 40, stock.c.dcinvtnflag != 30,stock.c.dcinvtnflag != 90))).order_by(stock.c.stockdate))
            ...
            for finalRow in stockData:
                if finalRow["dcinvtnflag"] == 3 or  finalRow["dcinvtnflag"] ==  9:
                    countresult = con.execute(select([invoice.c.invoicedate,invoice.c.invoiceno,invoice.c.custid]).where(and_(invoice.c.invoicedate <= endDate, invoice.c.invid == finalRow["dcinvtnid"])))
                    if countresult.rowcount == 1:
                        countrow = countresult.fetchone()
                        custdata = con.execute(select([customerandsupplier.c.custname]).where(customerandsupplier.c.custid == countrow["custid"]))
                ...
                if finalRow["dcinvtnflag"] == 4:
                    countresult = con.execute(select([delchal.c.dcdate,delchal.c.dcno,delchal.c.custid]).where(and_(delchal.c.dcdate <= endDate, delchal.c.dcid == finalRow["dcinvtnid"])))
                    if countresult.rowcount == 1:
                        countrow = countresult.fetchone()
                        custdata = con.execute(select([customerandsupplier.c.custname]).where(customerandsupplier.c.custid == countrow["custid"]))
                        custrow = custdata.fetchone()
                        dcinvresult = con.execute(select([dcinv.c.invid]).where(dcinv.c.dcid == finalRow["dcinvtnid"]))
                        if dcinvresult.rowcount == 1:
                            dcinvrow = dcinvresult.fetchone()
                            invresult = con.execute(select([invoice.c.invoiceno]).where(invoice.c.invid == dcinvrow["invid"]))
                ...
                if finalRow["dcinvtnflag"] == 7:
                    countresult = con.execute(select([func.count(drcr.c.drcrid).label('dc')]).where(and_(drcr.c.drcrdate <= endDate, drcr.c.drcrid == finalRow["dcinvtnid"])))
                    countrow = countresult.fetchone()
                    if countrow["dc"] == 1:
                        if  finalRow["inout"] == 9:
                            openingStock = float(openingStock) + float(finalRow["qty"])
        ...
        if productCode == "all":
            products = con.execute(select([product.c.openingstock,product.c.productcode,product.c.productdesc]).where(and_(product.c.orgcode == orgcode,product.c.gsflag == 7)))
            ...
            for row in prodDesc:
                ...
                stockRecords = con.execute(select([stock]).where(and_(stock.c.productcode == productCd,stock.c.orgcode == orgcode, or_(stock.c.dcinvtnflag != 20,stock.c.dcinvtnflag != 40, stock.c.dcinvtnflag != 30,stock.c.dcinvtnflag != 90))))
                stockData = stockRecords.fetchall()
                totalinward = totalinward + float(openingStock)
                for finalRow in stockData:
                    if finalRow["dcinvtnflag"] == 3 or  finalRow["dcinvtnflag"] ==  9:
...
def godownwisestockonhandfun(con, orgcode, endDate, stocktype, productCode, godownCode):
    try:
        ...
        if stocktype == "pg":
            ...
            goopeningStockResult = con.execute(select([goprod.c.goopeningstock]).where(and_(goprod.c.productcode == productCode,goprod.c.goid == godownCode, goprod.c.orgcode == orgcode)))
            gosRow =goopeningStockResult.fetchone()
            if gosRow!=None:
                gopeningStock = gosRow["goopeningstock"]
            else:
                gopeningStock = 0.00
            stockRecords = con.execute(select([stock]).where(and_(stock.c.productcode == productCode,stock.c.goid == godownCode,stock.c.orgcode == orgcode, or_(stock.c.dcinvtnflag != 40, stock.c.dcinvtnflag != 30,stock.c.dcinvtnflag != 90))).order_by(stock.c.stockdate))
            stockData = stockRecords.fetchall()
            ysData = con.execute(select([organisation.c.yearstart]).where(organisation.c.orgcode == orgcode) )
            ...
            for finalRow in stockData: 
                ...
                if finalRow["dcinvtnflag"] == 20:
                    countresult = con.execute(select([transfernote.c.transfernotedate,transfernote.c.transfernoteno]).where(and_(transfernote.c.transfernotedate <= endDate, transfernote.c.transfernoteid == finalRow["dcinvtnid"])))
                    if countresult.rowcount == 1:
                        countrow = countresult.fetchone()
                        if  finalRow["inout"] == 9:
                            gopeningStock = float(gopeningStock) + float(finalRow["qty"])
                ...
                if finalRow["dcinvtnflag"] == 18:
                    ...
                    if  finalRow["inout"] == 15:
                        ...
                        totaloutward = float(totaloutward) + float(finalRow["qty"])
                if finalRow["dcinvtnflag"] == 7:
                    countresult = con.execute(select([func.count(drcr.c.drcrid).label('dc')]).where(and_(drcr.c.drcrdate >= yearStart, drcr.c.drcrdate < startDate, drcr.c.drcrid == finalRow["dcinvtnid"])))
                    countrow = countresult.fetchone()
                    if countrow["dc"] == 1:
                        if  finalRow["inout"] == 9:
                            gopeningStock = float(gopeningStock) + float(finalRow["qty"])
        ...
        if stocktype == "pag":
            productCode = productCode
            products = con.execute(select([product.c.productdesc]).where(and_(product.c.productcode == productCode,product.c.orgcode == orgcode)))
            prodDesc =  products.fetchone()
            goopeningStockResult = con.execute(select([goprod.c.goopeningstock,goprod.c.goid]).where(and_(goprod.c.productcode == productCode, goprod.c.orgcode == orgcode)))
            ...
            for row in gosRow:
                ...
                if row["goopeningstock"]!=None:
                    ...
                    gopeningStock = 0.00
                godowns = con.execute(select([godown.c.goname]).where(and_(godown.c.goid == row["goid"],godown.c.orgcode == orgcode)))
                goName = godowns.fetchone()
                gn = goName["goname"]
                stockRecords = con.execute(select([stock]).where(and_(stock.c.productcode == productCode,stock.c.goid == row["goid"],stock.c.orgcode == orgcode, or_(stock.c.dcinvtnflag != 40, stock.c.dcinvtnflag != 30,stock.c.dcinvtnflag != 90))).order_by(stock.c.stockdate))
                stockData = stockRecords.fetchall()
                totalinward = totalinward + float(gopeningStock)
                for finalRow in stockData:    
                    if finalRow["dcinvtnflag"] == 4:
                    ...
                    if stockRow["dcinvtnflag"] == 7:
                        countresult = con.execute(select([func.count(drcr.c.drcrid).label('dc')]).where(and_(drcr.c.drcrdate >= yearStart, drcr.c.drcrdate < startDate, drcr.c.drcrid == stockRow["dcinvtnid"])))
                        countrow = countresult.fetchone()
                        if countrow["dc"] == 1:
                            if  stockRow["inout"] == 9:
            ...
            try:
                ...
                if projectCode == "":
                    if "orderflag" in self.request.params:
                        transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,projectcode,orgcode,invid,drcrid  from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and (drs ? '%s' or crs ? '%s') order by voucherdate DESC,vouchercode ;"%(calculateFrom, calculateTo, accountCode,accountCode))
                    else:
                        transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,projectcode,orgcode ,invid,drcrid  from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and (drs ? '%s' or crs ? '%s') order by voucherdate,vouchercode ;"%(calculateFrom, calculateTo, accountCode,accountCode))
                else:
                    if "orderflag" in self.request.params:
                        transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,projectcode,orgcode,invid,drcrid  from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and projectcode=%d and (drs ? '%s' or crs ? '%s') order by voucherdate DESC, vouchercode;"%(calculateFrom, calculateTo,int(projectCode),accountCode,accountCode))
                    else:
                        transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,projectcode,orgcode,invid,drcrid  from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and projectcode=%d and (drs ? '%s' or crs ? '%s') order by voucherdate, vouchercode;"%(calculateFrom, calculateTo,int(projectCode),accountCode,accountCode))
                ...
                for transaction in transactions:
                    ...
                    dcinfo = billwiseEntryLedger(self.con, orgcode, transaction["vouchercode"], transaction["invid"], transaction["drcrid"])
                    if dcinfo != "":
        ...
        if authDetails["auth"] == False:
            ...
            try:
                ...
                if side=="dr":
                    if projectCode == "":
                        if "orderflag" in self.request.params:
                            transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,projectcode,orgcode,invid,drcrid from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and (drs ? '%s') order by voucherdate DESC;"%(calculateFrom, calculateTo, accountCode))
                        else:
                            transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,projectcode,orgcode,invid,drcrid from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and (drs ? '%s') order by voucherdate;"%(calculateFrom, calculateTo, accountCode))
                    else:
                        if "orderflag" in self.request.params:
                            transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,projectcode,orgcode,invid,drcrid from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and projectcode=%d and (drs ? '%s') order by voucherdate DESC;"%(calculateFrom, calculateTo,int(projectCode),accountCode))
                        else:
                            transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,projectcode,orgcode,invid,drcrid from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and projectcode=%d and (drs ? '%s') order by voucherdate;"%(calculateFrom, calculateTo,int(projectCode),accountCode))
                ...
                if side=="cr":
                    if projectCode == "":
                        if "orderflag" in self.request.params:
                            transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,projectcode,invid,drcrid,,orgcode from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and (crs ? '%s') order by voucherdate DESC;"%(calculateFrom, calculateTo, accountCode))
                        else:
                            transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,projectcode,invid,drcrid,orgcode from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and (crs ? '%s') order by voucherdate;"%(calculateFrom, calculateTo, accountCode))
                    else:
                        if "orderflag" in self.request.params:
                            transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,projectcode,invid,drcrid,orgcode from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and projectcode=%d and (crs ? '%s') order by voucherdate DESC;"%(calculateFrom, calculateTo,int(projectCode),accountCode))
                        else:
                            transactionsRecords = self.con.execute("select vouchercode,vouchernumber,voucherdate,narration,drs,crs,prjcrs,prjdrs,vouchertype,lockflag,delflag,invid,drcrid,projectcode,orgcode from vouchers where voucherdate >= '%s'  and voucherdate <= '%s' and projectcode=%d and (crs ? '%s') order by voucherdate;"%(calculateFrom, calculateTo,int(projectCode),accountCode))
                    transactions = transactionsRecords.fetchall()
                    for transaction in transactions:
                        ledgerRecord = {"vouchercode":transaction["vouchercode"],"vouchernumber":transaction["vouchernumber"],"voucherdate":str(transaction["voucherdate"].date().strftime('%d-%m-%Y')),"narration":transaction["narration"],"status":transaction["lockflag"], "vouchertype":transaction["vouchertype"]}
                        ledgerRecord["Cr"] = "%.2f"%float(transaction["crs"][accountCode])
    ...
    def godownwisestockforgodownincharge(self):
        ...
        if authDetails["auth"]==False:
            ...
            try:
                ...
                prodcode=self.con.execute("select productcode as productcode from goprod where goid=%d and orgcode=%d"%(godownCode,orgcode))
                ...
                if prodcodelist==None:
                    ...
                    for productcode in prodcodelist:
                        ...
                        result=godownwisestockonhandfun(self.con, orgcode, endDate, stocktype, productCode, godownCode)
                        resultlist= result[0]["prodid"]=productCode
                        stocklist.append(result[0])

                    allprodstocklist=sorted(stocklist,key =lambda x:float(x['balance']))[0:5]
                    for prodcode in allprodstocklist:
                        proddesc=self.con.execute("select productdesc as proddesc from product where productcode=%d"%(prodcode["prodid"]))
                        proddesclist=proddesc.fetchone()
                        prodcodedesclist.append({"prodcode":prodcode["prodid"],"proddesc":proddesclist["proddesc"]})

                    return {"gkstatus":enumdict["Success"],"gkresult":allprodstocklist,"proddesclist":prodcodedesclist}
            except:
                return {"gkstatus":enumdict["ConnectionFailed"]}

...
def createtn(self):
    ...
    if authDetails["auth"] == False:
        ...
        try:
            ...
            result = self.con.execute(gkdb.transfernote.insert(),[dataset])
...
def getAllTn(self):
        ...
        if authDetails["auth"] == False:
            ...
            try:
                self.con = eng.connect()
                result = self.con.execute(select([gkdb.transfernote.c.transfernoteno, gkdb.transfernote.c.transfernotedate, gkdb.transfernote.c.transportationmode, gkdb.transfernote.c.productdetails, gkdb.transfernote.c.nopkt, gkdb.transfernote.c.recieved, gkdb.transfernote.c.fromgodown, gkdb.transfernote.c.togodown,gkdb.transfernote.c.orgcode]).order_by(gkdb.transfernote.c.transfernotedate))
                transfernote = []
                for row in result:
...
def deletePurchaseOrder(self):
        ...
        if authDetails["auth"]==False:
            ...
            try:
                self.con = eng.connect()
                dataset = self.request.json_body
                result = self.con.execute(gkdb.transfernote.delete().where(gkdb.transfernote.c.transfernoteno == dataset["transfernoteno"]))
                return {"gkstatus":enumdict["Success"]}
            except exc.IntegrityError:

     ...
     #       try:
                ...
                if row["icflag"]==3:
                    ...
                    if result.rowcount>0:
                        ...
                        invc["csflag"]=custname["csflag"]
                for item in items.keys():
                    result = self.con.execute(select([product.c.productdesc,product.c.uomid,product.c.gscode,product.c.gsflag]).where(product.c.productcode==item))
                    productname = result.fetchone()
                    if int(productname["gsflag"]) == 7:

...
class TestRollClose:
    ...
    def test_closebooks(self):
    	result = requests.get("http://127.0.0.1:6543/rollclose?task=closebooks&financialend=2016-03-31", headers=self.header)
        assert result.json()["gkstatus"] == 0
    def test_rollover(self):
        result = requests.get("http://127.0.0.1:6543/rollclose?task=rollover&financialend=2017-03-31&financialstart=2016-04-01", headers=self.header)
        assert result.json()["gkstatus"] == 0
